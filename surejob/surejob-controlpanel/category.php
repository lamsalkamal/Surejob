...
if (strlen($_SESSION['alogin'])==0) {
    ...
    $_SESSION['category-create-msg']="";
    $_SESSION['delmsg']="";
    $_SESSION['msgclass']="";

    if (isset($_POST['create-category'])) {
        $category=$_POST['category'];
        $sql=mysqli_query($conn, "insert into category(categoryName,updationDate) values('$category','$currentTime')");
        $_SESSION['category-create-msg']="Category Created !!";
        $_SESSION['msgclass']="success-msg";
    }

    if (isset($_GET['del'])) {
        $delQuery = mysqli_query($conn, "delete from category where id = '".$_GET['id']."'");
        if ($delQuery) {
    ...
    <div class="row">
        ...
        <div class="next-col col-md-9">
            <form name="Category" method="post" class="change-password-form">
                <h5 class="form-head">Category</h5>
                <hr />
                <?php if ($_SESSION['category-create-msg'] != '') { ?>
                <span class=<?php echo $_SESSION['msgclass']; ?>>
                    <?php echo htmlentities($_SESSION['category-create-msg']); ?>
                </span>
                <?php } ?>
                ...
                <hr />
                <button type="submit" class="btn btn-primary" name="create-category">Create Category</button>
            </form>

            <div class="manage-category">
                <h5 class="manage-category-head">Manage Categories</h5>
                <hr />
